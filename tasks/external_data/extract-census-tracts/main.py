...
def extract_census_tracts(request):
    """Verify census tract data exists in GCS raw_data bucket."""
    try:
        ...
        if not blobs:
            return {
                "success": False,
                "error": "No census tract data found in GCS",
            }, 404

        total_size_mb = sum(b.size for b in blobs) / (1024 * 1024)
        return {
            "success": True,
            "message": "Census tract data verified in GCS",
