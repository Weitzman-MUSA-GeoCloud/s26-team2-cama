...
def prepare_crime(request):
    """Convert crime CSV to JSON-L with lowercased field names."""
    storage_client = storage.Client()

    raw_bucket = "musa5090s26-team2-raw_data"
    prep_bucket = "musa5090s26-team2-prepared_data"
    prep_blob_path = "external_data/crime/data.jsonl"

    local_csv = "/tmp/crime.csv"
    local_jsonl = "/tmp/crime.jsonl"

    try:
        bucket = storage_client.bucket(raw_bucket)
        blobs = list(bucket.list_blobs(prefix="external_data/crime/"))
        csv_blob = next(
        ...
        if csv_blob is None:
            return {"success": False, "error": "No CSV file found in external_data/crime/"}, 404

        logging.info(f"Downloading {csv_blob.name}...")
        csv_blob.download_to_filename(local_csv)

        row_count = 0
        with open(local_csv, "r", encoding="utf-8") as infile, \
        ...
        return {
            "success": True,
            "message": f"Crime data converted to JSON-L ({row_count} rows)",
