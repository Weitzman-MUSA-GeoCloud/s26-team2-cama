...
def extract_recreation(request):
    """Verify recreation data exists in GCS raw_data bucket."""
    try:
        ...
        if not blobs:
            return {
                "success": False,
                "error": "No recreation data found in GCS",
            }, 404

        total_size_mb = sum(b.size for b in blobs) / (1024 * 1024)
        return {
            "success": True,
            "message": "Recreation data verified in GCS",
